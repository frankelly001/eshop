...
const ProductsLayout = ({allProducts, homePage}) => {
  ...
  const { dispatch } = useContext(UserContext)
    ...
    return (
      <div>
      <RenderLoader allData={allProducts} />
      ...
      <section className="eshop_body">
        <div className="eshop_body-content">
          ...
          {allProducts.map((product) => (
            <div
              key={product.id}
              className="eshop_product"
            >
              <div className="eshop_product-img">
              <LikeBtn product={product}/>
                <img onClick={() => navigate(`/products/${formatStr(product.title)}`)} src={product.image} alt={product.title} />
              </div>
              <div className="eshop_product-details">
                ...
                <button className='addtocart' onClick={()=>dispatch({type: "addToCart",id: product.id})}>Add to cart</button>
              </div>
            </div>
          ))}
        </div>
